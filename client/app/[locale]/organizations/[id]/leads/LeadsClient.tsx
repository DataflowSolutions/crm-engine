...
export default function LeadsClient({ leads: initialLeads, orgId, orgName, locale, permissions }: Props) {
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      if (searchTerm) {
        ...
        const matchesName = leadName.toLowerCase().includes(searchLower);
        
        // Also search in lead field values
        const matchesFields = lead.lead_field_values.some(fieldValue => 
          fieldValue.value.toLowerCase().includes(searchLower)
        );
        
        if (!matchesName && !matchesFields) return false;
      }
  ...
  // Calculate pagination based on filtered leads
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div>
                <select
                  ...
                  className="px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white cursor-pointer"
                >
        ...
        {!leads || leads.length === 0 ? (
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-16 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('noLeads', {default: 'No leads yet'})}</h3>
              <p className="text-gray-600 mb-8 text-lg">
                {t('noLeadsDescription', {default: 'Create your first lead to start managing your prospects, or import leads from a spreadsheet.'})}
              </p>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-16 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('noFilteredResults', {default: 'No leads found'})}</h3>
              <p className="text-gray-600 mb-8 text-lg">
                {t('noFilteredResultsDescription', {default: 'No leads match your current search and filter criteria. Try adjusting your filters.'})}
              </p>
              <button
                onClick={() => {
                  setSearchTerm('');
            ...
            <div className="px-6 py-5 border-b border-gray-200/50 flex items-center justify-between bg-gradient-to-r from-slate-50 to-gray-50">
              ...
              <div className="flex items-center gap-4">
                ...
                {filteredLeads.length > 0 && permissions.canDeleteLeads && (
                  ...
                  >
                    {selectedLeads.size === currentLeads.length ? t('deselectAll', {default: 'Deselect All'}) : t('selectAll', {default: 'Select All'})}
                  </button>
                )}
              </div>
            </div>
